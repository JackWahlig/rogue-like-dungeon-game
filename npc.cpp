...
void npc_next_pos_rand_tunnel(dungeon *d, character *c, pair_t next)
{
  ...
  } while (mappair(n) == ter_wall_immutable || mappair(n) == ter_wizard);
...
void npc_next_pos_line_of_sight(dungeon *d, character *c, pair_t next)
{
  ...
  if ((((npc *) c)->characteristics & NPC_PASS_WALL) &&
      mapxy(next[dim_x] + dir[dim_x], next[dim_y] + dir[dim_y]) != ter_wizard) {
                                       ...
                                       pair_t next)
{
  ...
  if(mapxy(dir[dim_x] + next[dim_x], dir[dim_y] + next[dim_y]) != ter_wizard){
...
void npc_next_pos(dungeon *d, npc *c, pair_t next)
{
  ...
  if(!c->bribed){
    ...
    int i, j;
    for(i = c->position[dim_x] - 1; i <= c->position[dim_x] + 1; i++){
      for(j = c->position[dim_y] - 1; j <= c->position[dim_y] + 1; j++){
	if((i != c->position[dim_x] || j != c->position[dim_y]) && charxy(i, j)){
	  if((charxy(i, j) != d->PC) && !((npc *)charxy(i, j))->bribed){
	    next[dim_x] = i;
	    next[dim_y] = j;
	    following = 0;
	  }
...
npc::npc(dungeon *d, monster_description &m) : md(m)
{
  ...
  do {
    ...
    i++;
  } while (d->character_map[p[dim_y]][p[dim_x]] || mappair(p) == ter_wizard);
  betrayed = 0;
  bribed = 0;
  greed = 1 + (rand() % 200);
  wealth = 1 + (rand() % 100);
...
uint32_t spongebob_is_alive(dungeon *d)
{
  static uint32_t initialized = 0;
  static std::vector<monster_description>::iterator i;

  if (!initialized) {
    for (i = d->monster_descriptions.begin();
         i->name != "SpongeBob SquarePants";
         i++)
      ;
    initialized = 1;
  }
