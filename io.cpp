...
uint32_t io_teleport_pc(dungeon *d)
{
  ...
  * not otherwise used.                                                 */
  ...
  if ((charpair(dest) && charpair(dest) != d->PC) || mappair(dest) == ter_wizard) {
...
void io_display_ch(dungeon_t *d)
{
  ...
  mvprintw(13, 33, " Gold:%5d ", d->PC->wealth);
  mvprintw(15, 27, " Hit any key to continue. ");
  refresh();
  getch();
  io_display(d);
}
...
static uint32_t io_inspect_monster(dungeon_t *d)
{
  ...
  mvprintw(0, 0, "Choose a monster or item.  't' to select; 'ESC' to cancel.");
  ...
  * not otherwise used.                                                 */
    ...
    cont = 1;
    if((charpair(dest) && charpair(dest) != d->PC) || objpair(dest) || mappair(dest) == ter_wizard){
      cont = 0;
    }
  } while ((c == 't' && cont) ||
           (c != 't' && c != 27 /* ESC */));
  ...
  if(charpair(dest)){
    ...
    io_display_obj_info(objpair(dest));
  } else if (mappair(dest) == ter_wizard){
    mvprintw(0, 0, "A mighty wizard, waiting for you to approach.");
  }
...
void io_talk(dungeon *d)
{
  ...
  //io_queue_message("Enter a direction to talk to an NPC");
  mvprintw(0, 0, "Enter a direction to talk to an NPC, Escape to quit");
  do {
    do{
      FD_ZERO(&readfs);
      FD_SET(STDIN_FILENO, &readfs);
    ...
    case 'y':
    case KEY_HOME:
      fail_code = d->PC->talk(d, 7);
      break;
    case '8':
    case 'k':
    case KEY_UP:
      fail_code = d->PC->talk(d, 8);
      break;
    case '9':
    case 'u':
    case KEY_PPAGE:
      fail_code = d->PC->talk(d, 9);
      break;
    case '6':
    case 'l':
    case KEY_RIGHT:
      fail_code = d->PC->talk(d, 6);
      break;
    case '3':
    case 'n':
    case KEY_NPAGE:
      fail_code = d->PC->talk(d, 3);
      break;
    case '2':
    case 'j':
    case KEY_DOWN:
      fail_code = d->PC->talk(d, 2);
      break;
    case '1':
    case 'b':
    case KEY_END:
      fail_code = d->PC->talk(d, 1);
      break;
    case '4':
    case 'h':
    case KEY_LEFT:
      fail_code = d->PC->talk(d, 4);
      break;
    case 27:
      fail_code = 0;
      break;
...
void io_push(dungeon *d)
{
  ...
  //io_queue_message("Enter a direction to talk to an NPC");
  mvprintw(0, 0, "Enter a direction to switch places with an ally, Escape to quit");
  do {
    do{
      FD_ZERO(&readfs);
      FD_SET(STDIN_FILENO, &readfs);
    ...
    case 'y':
    case KEY_HOME:
      fail_code = d->PC->push(d, 7);
      break;
    case '8':
    case 'k':
    case KEY_UP:
      fail_code = d->PC->push(d, 8);
      break;
    case '9':
    case 'u':
    case KEY_PPAGE:
      fail_code = d->PC->push(d, 9);
      break;
    case '6':
    case 'l':
    case KEY_RIGHT:
      fail_code = d->PC->push(d, 6);
      break;
    case '3':
    case 'n':
    case KEY_NPAGE:
      fail_code = d->PC->push(d, 3);
      break;
    case '2':
    case 'j':
    case KEY_DOWN:
      fail_code = d->PC->push(d, 2);
      break;
    case '1':
    case 'b':
    case KEY_END:
      fail_code = d->PC->push(d, 1);
      break;
    case '4':
    case 'h':
    case KEY_LEFT:
      fail_code = d->PC->push(d, 4);
      break;
    case 27:
      fail_code = 0;
      break;
